...
WHY THIS IS THE CORE QUESTION.  §3 gives the magnitude ratio exactly from the field,
...
WHY IT IS ASKED AGAIN.  §5 already reports that it does not, and answers it against three chosen
...
WHAT IS DIFFERENT HERE.  §4.1's method: hand the instrument the whole object and let it find the
...
This is the largest linear basis available: any functional §5 could have chosen is a vector in it,
...
THE CONTROL IS THE POINT.  The same read is run on `ln|det_up| - ln|det_dn|`, which §3 says IS an
...
def read_against_the_field(K, **kw):
    ...
    def scored(w, draws=30):
        ...
        true = float(E.carriage(X, w).carried)
        nulls = [float(E.carriage(X, w[rng.permutation(len(w))]).carried) for _ in range(draws)]
...
if __name__ == "__main__":
    ...
    print()
    print("The CONTROL column is ln|det_up| - ln|det_dn| read against the same frame. Section 3")
    ...
    print("on the full field basis, which contains every functional section 5 chose.")
