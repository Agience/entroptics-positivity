...
if __name__ == "__main__":
    ...
    print("derivative to read, while the deficit has already moved -- by 162x its own seed spread.")
    print()
    print("|z| is the instrument's resolution against its re-pairing null, which is centred at")
    print("strength = 0.  For one column pair it is exactly |strength| sqrt(n - 1), so it says")
    print("the channels are coupled, not that the deficit has left zero.  Read the seed spread for")
    print("that.  |z| is reported because `resolved` gates on it.")
    print()
