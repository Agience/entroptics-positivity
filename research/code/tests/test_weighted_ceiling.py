"""§7's ceiling, gated: every weighted read is capped at n<sgn>^2.
...
def test_the_effective_sample_size_is_exactly_n_times_mean_sign_squared(n, p):
    ...
    assert EA.evidence_ceiling(X, w).effective_n == pytest.approx(n * float(w.mean()) ** 2, rel=1e-12)
...
def test_the_ceiling_does_not_depend_on_the_frames():
    ...
    which is why the bound applies to the coupling of §7 as much as to `carriage` itself.
    """
    ...
    for seed, shape in ((1, (600, 8)), (2, (600, 32)), (3, (600, 3))):
        ...
        v = EA.evidence_ceiling(X, w).effective_n
        if base is None:
            base = v
...
def test_the_ceiling_collapses_as_the_sign_problem_worsens():
    ...
    for p in (0.99, 0.8, 0.6, 0.51):
        ...
        ess.append(EA.evidence_ceiling(np.random.default_rng(0).standard_normal((n, 8)), w).effective_n)
