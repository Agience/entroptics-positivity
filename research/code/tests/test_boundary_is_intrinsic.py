"""The boundary of §7 is intrinsic on one side and mechanism-dependent on the other.
...
def test_the_complex_frame_and_its_real_embedding_agree():
    ...
    direct = EA.channel_alignment(A, B)
    embedded = EA.channel_alignment(embed(A), embed(B))
...
def test_the_instrument_does_not_recover_the_split_unaided(phi_over_pi):
    ...
    aided = [float(EA.channel_alignment(np.real(A), np.real(B)).strength),
             float(EA.channel_alignment(np.imag(A), np.imag(B)).strength)]
...
def test_an_exact_channel_relation_does_not_determine_the_sign():
    """§5 route B: the channels satisfy G_dn = 1 - G_up exactly AND the weight carries a phase.
    ...
    def probe(Kmat, beta=8.0, U=4.0, dtau=0.125, n=200, seed=5):
        ...
        return (float(np.abs(A + B - 1.0).max()),
                ...
                float(EA.channel_alignment(emb(A), emb(B)).strength))
...
def test_the_phase_problem_is_a_common_mode_and_is_readable_one_sidedly():
    ...
    for K in (ring(5, np.pi / 2), tri_ladder(8, np.pi / 2)):
        ...
        assert float(EA.weight_cloud(u).resultant) == pytest.approx(truth, abs=1e-9)
